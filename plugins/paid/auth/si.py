...
Result -» <b>Declined! ❌</b>
Message -» <b>incorrect_number</b>
Gateway -» <b>Stripe 4</b>
<b>———»Details«———</b>
Bin -» <b>{bin}</code> - <b>{bin_data['data']['level']}</b> - <b>{bin_data['data']['type']}</b>
Bank -» <b>{bin_data['data']['bank']}</b>
Country -» <b>{bin_data['data']['countryInfo']['code']}({bin_data['data']['countryInfo']['emoji']})</b>
<b>———-»Info«-———-</b>
Time taken -» {get_time_taken(started_time)}'s
Checked by -»  <b><a href="tg://user?id={message.from_user.id}">{message.from_user.first_name}</a> [<i>{find['role']}</i>]</b>
Edited By:-<code>Tabah_Rs</code>"""
...
Edited By:-<code>Tabah_Rs</code>"""
                                    ...
                                    try:
                                        ...
                                        if response is None:
                                            await msg.edit_text("PROXY DEAD PLEASE REPORT TO OWNER Tabah_Rs")
                                        else:
...
Edited By:-<code>Tabah_Rs</code>"""
        ...
        await msg.edit_text("PROXY DEAD PLEASE REPORT TO OWNER <code>Tabah_Rs</code>")
